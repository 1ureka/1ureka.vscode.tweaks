...
- **前後端通訊架構**
  ...
  - **右鍵選單橋接機制**：VSCode 的 webview 無法處理自身的右鍵選單事件，因此實作了命令橋接系統
    - 當使用者在 webview 上按右鍵並選擇命令時，延伸主機透過橋接訊息通知前端「要執行什麼操作」
    - 前端收到通知後，根據自身的 UI 狀態（如選取的項目）準備完整參數，再透過正常 API 通道請求延伸主機執行
    - 通知也可以是純與檢視有關，當前端認為該意圖不需要與延伸主機互動時，可以直接在前端處理

...
- 嚴格遵守單向資料流原則，就連最後更新時間也由後端決定，達成真正的 `UI = f(state)`，也根絕了所謂不同步的問題
- 實現完整路由，包括瀏覽資料夾、回到上層、麵包屑導航、開啟檔案到 VsCode 中等
...
在開發該專案的過程中，由於 VsCode 插件的特殊之處，前後端的溝通是沒有流量壓力的，因此我發現了一種有趣的 pattern

我認為，相較於OOP將資料與方法都封裝的作法，對於一個模組，它應該提供以下幾種東西:

參數定義: 一個輸入參數的型別定義
狀態定義: 一個該模組所產出的狀態型別定義
純函式: 一些只存在於內部的純函式，接收完整或部分的參數定義，並回傳完整的狀態定義
溝通介面: 一個事件分派機制，可以讓外部呼叫，其為分派到對應的純函式，並將結果同樣以新事件回傳給外部

當今天模組A想要使用模組B的功能時，對於模組A，其應該要實作:

狀態容器: 導入模組B的狀態定義，實做一個必須能讓模組A的其他部分訂閱的容器 (ex. Zustand store)
調用邏輯: 導入模組B的參數定義，並實作一組調用邏輯，可以呼叫模組B的溝通介面，並且呼叫後就不理
唯獨與監聽: 調用邏輯不該等待結果，而是由狀態容器監聽模組B的事件，當事件來時更新狀態

總而言之，對於被使用方，其只定義狀態，但邏輯都是無狀態的
而對於使用方，其才是狀態的擁有者，並且實作監聽與調用邏輯

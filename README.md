...
- 嚴格遵守單向資料流原則，並且利用了 JS 的事件循環機制，保證所有操作的原子化與順序一致性，確保資料安全
- 全局載入指示器，利用佇列與延遲機制，任何的載入行為都會被追蹤到，且只在需要時才顯示給使用者
- 用 CSS 與最基礎的 MUI 元件從零打造的表格框架，確保其擁有最強的樣式彈性與針對檔案系統顯示的最佳化

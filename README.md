...
| 檔案攔截     | 避免某些檔案類型導致編輯器凍結或無法正確顯示，改為顯示圖示與檔名，並提供以預設應用程式開啟的按鈕，包括下列檔案類型                                                                        |
| PDF 檔案攔截 | 攔截 PDF 檔案的開啟行為，避免 VS Code 以文本編輯器載入二進位內容導致凍結                                                                                                                  |
| 媒體檔案攔截 | 攔截影片與音訊檔案的開啟行為，取代 VS Code 內建的播放體驗（無聲音、無法與系統整合等問題）                                                                                                 |
| 圖像檔案     | 本擴展**不攔截**圖像檔案的開啟，若想要更好的圖像預覽體驗，可以考慮使用 [vs-preview](https://github.com/1ureka/vs-preview)                                                                 |
| 白名單機制   | 預設允許 `.blend`, `.spp`, `.html`, `.pdf`, `.mp4`, `.avi`, `.mkv`, `.mov`, `.wmv`, `.flv`, `.webm`, `.mp3`, `.wav`, `.flac`, `.aac`, `.ogg`, `.wma`, `.m4a` ，其餘副檔名會彈出確認對話框 |

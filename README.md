...
- **表格框架設計**
  ...
  - 使用 CSS 與基礎 MUI 元件從零打造，提供最大的樣式彈性與針對檔案系統顯示的最佳化
  - 採用雙層 Box 容器實現視覺與互動分離：背景層負責點擊與選取效果，內容層負責實際資料呈現
  - 支援完整的表格操作：排序、篩選、分頁、選取，所有操作皆有響應式視覺回饋

- **前後端通訊架構**

  - 前端發送請求時透過請求佇列系統確保操作依序執行，從根本上避免競態條件
  - 佇列系統同時作為載入狀態的可靠來源：只要佇列中有待處理的請求，loading 狀態就為 true
  - 全局載入指示器利用 CSS 動畫延遲顯示，避免瞬間完成的操作造成閃爍，提升使用體驗

- **三層狀態管理**

  - 前端採用 Zustand 狀態管理，將資料分為三個層次，形成單向依賴鏈
  - 實際資料層 (Data Store)：儲存延伸主機回傳的檔案系統原始資料
  - 檢視狀態層 (View Store)：儲存使用者的檢視偏好（排序欄位、篩選條件、分頁、選取狀態等）
  - 檢視資料層 (View Data Store)：根據實際資料與檢視狀態計算得出，最終呈現在表格中的資料

- **依賴鏈與單向資料流**

  - 利用 Zustand 的同步訂閱機制與 JavaScript 事件循環特性，建立依賴鏈自動更新系統
  - 當實際資料更新時，自動觸發檢視狀態驗證（確保分頁合理性、清理無效選取）
  - 檢視狀態驗證完成後，自動觸發檢視資料重新計算（篩選 → 排序 → 分頁 → 附加圖示）
  - 整個更新鏈在單一事件循環內同步完成，保證原子性與一致性，無論更新來源是使用者操作或檔案系統變更
  - 依賴鏈同時也是彈性的，但保證了資料流向的單一性，比如:

    - 後端回傳新資料 → 驗證分頁與選取狀態 → 更新檢視資料
    - 使用者改變篩選條件 → 更新檢視資料
    - 以上兩種操作由於在前端實際上是同步執行的，因此仍是原子且一致的

- **計算與渲染分離**
  - 所有資料轉換邏輯（篩選、排序、分頁）完全在狀態層的訂閱回調中執行
  - React 元件僅負責讀取最終狀態並渲染，避免在渲染階段進行昂貴的計算
  - 整個專案一個 `useEffect` 也沒有，真正做到 React 所說的 `UI = f(state)`
  - 這種架構確保了效能最佳化，同時讓元件保持簡潔與可維護性
